...
from _common import get_installation_token_for_org, github_request, require_env, require_secret
    ...
    if isinstance(payload_data, list):
        for item in payload_data:
            if isinstance(item, dict):
                dispatch(token, org, orchestrator_repo, item)
        return 0
    if isinstance(payload_data, dict):
        dispatch(token, org, orchestrator_repo, payload_data)
        return 0
